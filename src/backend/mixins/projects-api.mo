...
mixin (
  ...
  public func incrementCloneCount(id : Common.ProjectId) : async ?ProjectTypes.Project {
    let now : Common.Timestamp = Int.abs(Time.now()) / 1_000_000;
    switch (projects.get(id)) {
      case (?p) {
        let updatedMeta : ProjectTypes.MetadataUpdate = {
          tags = p.metadata.tags;
          priority = p.metadata.priority;
          maturity = p.metadata.maturity;
          attentionFlag = p.metadata.attentionFlag;
          builderMode = p.metadata.builderMode;
        };
        let updated = ProjectLib.setMetadata(p, updatedMeta, now);
        projects.add(id, updated);
        ?updated;
      };
      case null { null };
